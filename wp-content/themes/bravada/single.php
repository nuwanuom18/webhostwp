...
<?php if ( 1 == cryout_get_option ('theme_singlenav') ) : ?>
	<nav id="nav-below" class="navigation">
		<?php
			...
			$default = '<img src="' . esc_url( get_header_image() ) . '" alt="" />';
			$bravada_prevthumb = (isset($brava_prevpost->ID) && get_the_post_thumbnail( $brava_prevpost->ID) ) ? get_the_post_thumbnail( $brava_prevpost->ID, 'large' ) : $bravada_prevthumb = $default;
			$brabada_nextthumb = (isset($bravada_nextpost->ID) && get_the_post_thumbnail( $bravada_nextpost->ID) ) ? get_the_post_thumbnail( $bravada_nextpost->ID, 'large' ) : $brabada_nextthumb = $default;
		?>
