...
if ( empty( $image ) && empty( $title ) && empty( $content ) && empty( $lpslidercta1text ) && empty( $lpslidercta2text ) ) return; ?>
	...
	<div class="lp-staticslider">
		<?php if ( ! empty( $image ) ) { ?>
			<img class="lp-staticslider-image" alt="<?php echo esc_attr( $title ) ?>" src="<?php echo esc_url( $image ); ?>" loading="lazy">
		<?php } ?>
	...
	<section id="lp-blocks<?php echo absint( $sid ) ?>" class="lp-blocks lp-blocks<?php echo absint( $sid ) ?> lp-blocks-rows-<?php echo esc_attr( apply_filters('bravada_blocks_perrow', $pagecount, $sid) ) ?>">
		<?php if(  ! empty( $maintitle ) || ! empty( $maindesc ) ) { ?>
			<header class="lp-section-header">
				<?php if( ! empty( $maintitle ) ) { ?><h3 class="lp-section-title"> <?php echo do_shortcode( wp_kses_post( $maintitle ) ) ?></h3><?php } ?>
    ...
    if ( $custom_query->have_posts() ) : ?>
		<section id="lp-boxes-<?php echo absint( $sid ) ?>" class="lp-boxes lp-boxes-<?php echo absint( $sid ) ?> <?php  echo esc_attr( $animated_class ) ?> lp-boxes-rows-<?php echo absint( $options['theme_lpboxrow' . $sid] ); ?>">
			<?php if( $options['theme_lpboxmaintitle' . $sid] || $options['theme_lpboxmaindesc' . $sid] ) { ?>
				<header class="lp-section-header">
					<?php if ( ! empty( $options['theme_lpboxmaintitle' . $sid] ) ) { ?> <h3 class="lp-section-title"> <?php echo do_shortcode( wp_kses_post( $options['theme_lpboxmaintitle' . $sid] ) ) ?></h3><?php } ?>
    		...
    		<?php while ( $custom_query->have_posts() ) :
		            ...
		            $box['content'] = $excerpt;
		            list( $box['image'], ) = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'bravada-lpbox-' . $sid );
			...
			<div class="lp-box box<?php echo absint( $colno ); ?> ">
				<div class="lp-box-inside">
					<div class="lp-box-image lpbox-rnd<?php echo absint( $randomness[$colno%8] ); ?>">
                        <a class="lp-box-imagelink" tabindex="-1" href="<?php echo esc_url( $link ); ?>" <?php echo esc_attr( $target ); ?>><span class="screen-reader-text"> <?php echo esc_attr( $title ); ?></span>  </a>
						<?php if( ! empty( $image ) ) { ?><img alt="<?php echo esc_attr( $title ) ?>" src="<?php echo esc_url( $image ) ?>" loading="lazy"> <?php } ?>
					</div>
			...
			<div class="lp-text-background">
			 	<?php if( ! empty( $data['image'] ) ) { ?>
			         <div class="lp-text-image">
			             <img alt="<?php echo esc_attr( $data['title'] ) ?>" src="<?php echo esc_url( $data['image'] ); ?>" loading="lazy">
			 		</div>
			     <?php } ?>
			 	<div class="lp-text-card">
			 		<?php if( ! empty( $data['title'] ) ) { ?><h3 class="lp-text-title"><?php echo wp_kses_post( do_shortcode( $data['title'] ) ) ?></h3><?php } ?>
			 		<?php if( ! empty( $data['text'] ) ) { ?><div class="lp-text-content"><?php echo wp_kses_post( do_shortcode( $data['text'] ) ) ?></div><?php } ?>
			 	</div>
